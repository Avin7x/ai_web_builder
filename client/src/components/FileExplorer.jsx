...
function TreeItem ({node, activeFile, onFileSelect, depth = 0}){
    ...
    return (
        <button onClick={() => { console.log("Clicked:", node.path); onFileSelect(node.path)}} 
        className={`w-full flex items-center gap-2 py-1.5 text-xs transition-colors rounded-md cursor-pointer ${isActive ? "bg-zinc-100 text-zinc-950 font-medium" : "text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900 "}`} style={{paddingLeft: `${depth * 12 + 8}px`}}>
...
const FileExplorer = ({files, activeFile, onFileSelect}) => {
  ...
  return (
    <div className="py-2 overflow-y-auto hide-srollbar">
        <p className="px-3 py-1.5 text-[10px] font-semibold uppercase
         tracking-widest text-zinc-400">Files</p>
         {tree.map(node => (
